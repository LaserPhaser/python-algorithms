...
class HashChain(object):
    ...
    def _write_search_result(self, was_found):
        """
        Function for printing Yes/No if result was found
    ...
    def find(self, data):
        ...
        """
        hash_string, ind = self._check_data(data)
        self._write_search_result(ind != -1)
    ...
    def check(self, idx):
        ...
        Args:
            idx: index in hash
        """
        self._write_chain(cur for cur in reversed(self.elements[idx]) if self._hash_func(cur) == idx)
